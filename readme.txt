...
Например, следующий вариант может вместить 16 профилей в 4 бита : 0x00/0xf0, 0x10/0xf0, ..., 0xf0/0xf0
...
Однако, практически все роутеры устанавливают правило iptables "-m state --state INVALID -j DROP" или
"-m conntrack --ctstate INVALID -j DROP", которое блокирует передачу пакетов с неверной чексуммой.
Если удастся избавиться от подобного правила, пакеты с неверной чексуммой начнут проходить NAT.
Чексуммы вовсе не будут считаться в conntrack, если выполнить "sysctl -w net.netfilter.nf_conntrack_checksum=0",
правило INVALID срабатывать не будет.

...
Например, следующий вариант может вместить 16 профилей в 4 бита : 0x00/0xf0, 0x10/0xf0, ..., 0x0f/0x0f
...
Сам по себе Linux NAT не проверяет чексумму в транспортном хедере, ему не важны tcp options.
Однако, многие роутеры, включая android, устанавливают правило iptables "-m state --state INVALID -j DROP",
которое блокирует передачу пакетов с неверным заголовком или чексуммой. Если удастся избавиться от подобного правила,
пакеты с неверной чексуммой начнут проходить NAT.
...
Не факт, что все NAT будут пропускать невалидные пакеты.
...
К сожалению, стоковая прошивка скорее всего не даст поменять правила iptables, и вы не сможете убрать проверку на invalid state,
если она присутствует. Openwrt ее не задействует, инвалидные пакеты проходят NAT.
